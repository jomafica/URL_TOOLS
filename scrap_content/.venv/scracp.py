...
def data_collection(data):
    ...
    for art in data:
        ...
        regex_id = re.search(r'(?<=(id\=\"post\-))[^\"]*.*?', str(art))
        ...
        if src_img or src_img2:
            ...
            json_underline["id"] = regex_id.group(0)


            # Dic for content
            json_underline["title"] = title.group(0)
            ...
            json_underline["url"] = url.group(0)

            if src_img:
                json_underline["img"] = src_img.group(0)
            if src_img2:
                json_underline["img"] = src_img2.group(0)

            # Append Dict above under "content"

            json_body_content.append(json_underline)
            json_line["articles"] = json_body_content
            json_body_temp.append(json_line)

    #Remove duplicates s   
    for i in range(len(json_body_temp)): 
        if json_body_temp[i] not in json_body_temp[i + 1:]: 
            json_body = (json_body_temp[i]) 
...
if __name__ == "__main__":
    ...
    @app.route('/api/v1/resource/1', methods=['GET'])
    def api_main():
